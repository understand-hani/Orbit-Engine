...
- [x] 默认后端 URL 改为 `https://telephony-locator-billing-empirical.trycloudflare.com`。
...
- [x] 完成 Day2 iOS 代码线：默认后端 URL 更新为 `https://telephony-locator-billing-empirical.trycloudflare.com`，仍保留 `UserDefaults` / 设置页覆盖。
...
- [x] 后端默认 tunnel 多次随测试更新，当前默认 URL 为 `https://telephony-locator-billing-empirical.trycloudflare.com`，仍保留 `UserDefaults` 覆盖和旧默认 URL 迁移。
